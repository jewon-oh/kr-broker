...
| 결과 행의 시각 | 통합 구조(`Ticker`, `Trade`, `Order` 등)에 `timestamp`와 `datetime`이 있습니다 | 통합 구조는 같습니다. 다만 KB증권의 `Ticker`, `OrderBook`, `Order`는 응답의 시각 형식을 확인하지 못해 `timestamp`가 비어 있고, `fetchMyTrades`의 `Trade`는 체결 행에 시각이 없어 비어 있습니다. 이 결과를 시각으로 정렬하거나 `since`로 거르는 코드는 KB증권에서 쓸 수 없습니다. 한국투자증권과 KB증권 국내의 `fetchTrades`는 체결에 시각만 와서 일봉으로 날짜를 정하고, 날짜를 정하지 못한 행은 `timestamp`를 비웁니다. 증권사 고유 조회는 국내 행에만 `timestamp`를 넣고 해외 행은 날짜의 시간대를 확인하지 못해 비워 둡니다 |
