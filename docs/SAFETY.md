...
## KB증권 잔고에서 못 읽은 시장의 종목은 미확인입니다

KB증권 `fetchBalance()`는 보유를 일부만 읽어도 던지지 않습니다. 이때 `info.readStatus`가 `PARTIAL`이고 읽지 못한 시장(`KR`, `US`)이 `info.unreadMarkets`에 있습니다. 읽지 못한 시장에서 목록에 없는 종목을 미보유로 처리하지 마십시오. 다음 조회에서 보유를 다시 확인하십시오. 미국 시장은 읽었는데 달러 예수금 행을 가리지 못하면 `USD` 항목이 없고 `info.unreadCurrencies`에 `USD`가 있습니다. 이때 `readStatus`는 `COMPLETE`일 수 있으므로, 달러 현금을 0으로 두지 말고 모르는 값으로 다루십시오.

라이브러리는 해외 잔고평가(`SPQM2226`) 응답의 모양을 알아봤을 때만 미국 보유(`US`)를 읽은 것으로 봅니다. 이 응답에는 배열이 둘 있습니다. 두 배열은 통화별 예수금 그리드와 종목 그리드입니다.

- 응답에 예수금 그리드가 있거나 종목 그리드에 빈 행만 있으면 보유가 0건이어도 읽은 것으로 봅니다.
- 응답에 라이브러리가 알아보지 못한 배열이 있으면 `US`를 못 읽은 시장으로 둡니다. 이전 조회에서 보유를 읽은 인스턴스도 마찬가지입니다. 이 규칙은 필드 이름이 바뀐 종목 그리드를 보유 없음으로 읽지 않기 위한 것입니다. 그래서 증권사가 명세에 없는 배열을 보내면 보유를 읽었어도 `PARTIAL`입니다. 라이브러리는 그 배열의 필드 이름을 경고 로그에 남깁니다.
- 종목코드는 있는데 수량을 읽지 못한 행이 있을 때도 `US`를 못 읽은 시장으로 둡니다.
