...
KB증권 `fetchBalance()`는 보유를 일부만 읽어도 던지지 않습니다. 이때 `info.readStatus`가 `PARTIAL`이고 읽지 못한 시장(`KR`, `US`)이 `info.unreadMarkets`에 있습니다. 읽지 못한 시장에서 목록에 없는 종목을 미보유로 처리하지 마십시오. 다음 조회에서 보유를 다시 확인하십시오.
