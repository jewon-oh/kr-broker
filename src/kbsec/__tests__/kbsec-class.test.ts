...
describe('describe() — 선언', () => {
    ...
    it('has 표 — 지원하는 것과 지원하지 않는 것을 밝힌다', () => {
        expect(exchange.has).toMatchObject({
            ...
            fetchOrder: true, fetchOpenOrders: true, fetchMyTrades: true, fetchTradingFee: 'emulated',
            fetchMarketCalendar: true,
            fetchMarkets: false, fetchClosedOrders: false, fetchTickers: false, createConditionalOrder: false,
        });
    });

    it('지원하지 않는 메서드는 NotSupported 다', async () => {
        await expect(exchange.fetchClosedOrders()).rejects.toThrow(NotSupported);
