 ...
 * ## 왜 따로 두는가
 *
 * 어댑터는 이 TR 의 행을 네 곳(체결 확정·결제대기 매도 차감·미체결 조회·해외 체결)에서
 * 각자 읽었고, 네 곳 모두 KB 가 **주지 않는 이름**(`ccls_q`·`trd_clsf`)을 읽고 있었다.
 * 그래서 체결 확정은 전부 "체결 미확인"(filled 0) 이었고, 결제대기 매도
 * 차감은 아무것도 빼지 않아 판 종목이 계속 보유로 표시됐다(실서버 실측).
 *
 * `ccls_q`·`trd_clsf` 는 **다른 TR**(SSQM0006/SSQM2121 의 입력, SSQM2121 의 출력)에서는
 * 실재하는 이름이라 그대로 옮겨 적은 것으로 보인다. 이름은 한 곳에서만 알고, 소비처는
 * 이 파서의 결과만 본다.
 *
 * ## 정본 — KB 공식 스펙(kbsecurities/kb-openapi `samples.generated.json`, outputSpec)
 *
 * | 뜻 | 국내 SSQM2341 | 해외 SPQM2103 | 종전 오독 |
 * |---|---|---|---|
 ...
 *
 * 종전 이름은 **폴백으로 남긴다** — 스펙 이름이 없을 때만 읽는다. 명세와 실응답이
 * 어긋난 전례가 있어 한 번에 끊지 않는다.
 *
 ...
 *
 * 이름이 "총체결수량" 이라 오래 **누적**으로 읽었는데, 라이브 프로브가 아니라고 답했다
 * (종목코드와 주문번호는 공개를 위해 가공한 값, `SSQM2341`):
 *
 * ```
 ...
 * 체결로 읽으면 낸 주문에 매칭되지 않는다 — 실제로 그래서 분할체결된 매도가 **첫 행의 수량만**
 * 체결로 확정됐고, 남은 수량을 다시 팔려다 `증거수량이 부족 [1951]` 로 반복 거부돼 청산 재시도
 * 상한까지 갔다. 호출하는 쪽의 기록에는 그 수량이 열린 포지션으로 남았고 실계좌에는 0주다.
 ...
 *
 * ## 방향 이름의 값은 라이브 프로브로 확정 예정
 *
 * 스펙은 이름(문자열)이라고만 말한다. `'매도'` 를 포함하면 매도, `'매수'` 를 포함하면
 ...
 * **유령청산 → 재매수** 로 이어진다. 프로브에서 값이 확인되면 그때 추가한다.
 */
...
export interface KbsecFillRow {
     ...
     */
    cost: number;
    /** 정렬용 체결시각 문자열(`HHMMSS[ss]`). 없으면 '' — 원래 순서를 유지한다. */
    seq: string;
    /**
