...
describe('스키마 검사기가 어긋난 자료를 잡는다', () => {
    ...
    it('구현으로 적은 셀이 미구현 API 를 가리킴', () => {
        const missing = coverage.brokers.toss.apis.find((a) => a.status === 'missing')?.id ?? '';
        expect(errorsOf((c) => { c.features.features[0].cells.toss.apis = [missing]; })).toContain('미구현 API를 가리킵니다');
    });
