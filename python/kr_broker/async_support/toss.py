...
import logging
import math
from typing import TYPE_CHECKING, Any, Awaitable, Callable, Dict, List, Optional, Set, Tuple, cast
...
class toss(Exchange, ImplicitAPI):
    ...
    def _market_from_symbol(self, symbol: str) -> MarketInterface:
    ...
    @staticmethod
    def _country_of(market: MarketInterface) -> str:
    ...
    def _assert_krx_tick_aligned(self, market: MarketInterface, price: Any, method: str) -> None:
    ...
    def _order_price_string(self, market: MarketInterface, price: Any) -> Str:
    ...
    async def fetch_market_investor_trading(self, market: str, interval: str = '1d', limit: int = 5,
    ...
    async def fetch_investor_trading(self, market: str, interval: str = '1d', limit: int = 5,
        ...
        return await self.fetch_market_investor_trading(market, interval, limit, params)
    ...
    async def fetch_market_sessions(self, market: str, params: Optional[Dict[str, Any]] = None) -> Any:
    ...
    async def fetch_market_calendar(self, params: Any = None, legacy_params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        if isinstance(params, str):
            ...
            return await self.fetch_market_sessions(params, legacy_params)
        market = self._calendar_market(params)
        value = await self.fetch_market_sessions(market, self.omit(params or {}, 'market'))
    ...
    async def current_kr_session(self, now_ms: Optional[int] = None) -> Optional[str]:
        ...
        try:
            session = find_kr_session(await self.fetch_market_sessions('KR'), now_ms)
    ...
    async def current_us_session(self, now_ms: Optional[int] = None) -> Optional[str]:
        ...
        try:
            session = find_us_session(await self.fetch_market_sessions('US'), now_ms)
            return session if session is not None else 'closed'
        except Exception:
    ...
    async def fetch_tickers(self, symbols: Strings = None, params: Optional[Dict[str, Any]] = None) -> Tickers:
        ...
        ids = [cast(str, self.market(symbol)['id']) for symbol in unified]
        rows: List[Any] = []
        batch = 200
    ...
    def parse_ohlcv(self, ohlcv: Any, market: Market = None) -> List[Any]:
        return [
    ...
    async def fetch_balance(self, params: Optional[Dict[str, Any]] = None) -> Balances:
        ...
        if currency is None or symbol is not None:
            ...
            holdings = self.unwrap(await self.private_account_get_holdings({'symbol': market_id} if market_id is not None else {}))
            ...
            if market_id is not None and any(item.get('symbol') == market_id and (self.safe_number(item, 'quantity') or 0) > 0
                ...
                sellable = {market_id: await self._fetch_sellable_quantity(market_id)}
    ...
    async def _fetch_sellable_quantity(self, market_id: str) -> float:
        """매도 주문에 즉시 쓸 수 있는 수량(`GET /sellable-quantity`). 값이 없거나 숫자가 아니면 0 이 아니라 모르는 것이므로 던진다."""
        response = self.unwrap(await self.private_account_get_sellable_quantity({'symbol': market_id}))
    ...
    async def edit_order(self, id: str, symbol: str, type: str, side: str, amount: Num = None, price: Num = None,
        ...
        gate = await self._check_orderable_session(symbol, country, {'isMarket': type == 'market', 'useAmountBased': False, 'quantity': 0}, None)
        if gate is not None:
    ...
    async def create_market_buy_order_with_cost(self, symbol: str, cost: float, params: Optional[Dict[str, Any]] = None) -> Order:
    ...
    async def _check_orderable_session(self, symbol: str, country: str, form: Dict[str, Any], side: Optional[str]) -> Optional[str]:
        ...
        async def auction() -> Optional[str]:
            if side is None or not await self.is_option_enabled('blockAuctionBuys'):
                return None
    ...
    def _build_created_order(self, draft: Dict[str, Any], status: Str, still_open: Optional[bool] = None,
                             snapshot: Optional[Dict[str, Any]] = None, raw: Any = None) -> Order:
    ...
    async def _conditional_order_fields(self, market: MarketInterface, amount: float, plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _create_conditional_order(self, market: MarketInterface, type: str, side: str, amount: float, price: Num,
                                  params: Dict[str, Any]) -> Order:
    ...
    async def _modify_conditional_order(self, id: str, market: MarketInterface, type: str, side: str, amount: float, price: Num,
                                  params: Dict[str, Any]) -> Order:
    ...
    def _parse_conditional_order(self, conditional: Dict[str, Any], market: Market = None) -> Order:
    ...
    async def _fetch_open_conditional_rows(self, market: Market) -> List[Dict[str, Any]]:
    ...
    async def fetch_closed_orders(self, symbol: Str = None, since: Int = None, limit: Int = None,
        ...
        return await self._fetch_ended_orders('closed', symbol, since, limit, params)
    ...
    async def fetch_canceled_orders(self, symbol: Str = None, since: Int = None, limit: Int = None,
        ...
        return await self._fetch_ended_orders('canceled', symbol, since, limit, params)

    async def _fetch_ended_orders(self, status: str, symbol: Str, since: Int, limit: Int,
                            params: Optional[Dict[str, Any]]) -> List[Order]:
        market = self.market(symbol) if symbol is not None else None
        params = {} if params is None else params
        until = self.safe_integer(params, 'until')
    ...
    async def _fetch_closed_order_rows(self, market: Market, since: Int, limit: Int,
    ...
    async def fetch_my_trades(self, symbol: Str = None, since: Int = None, limit: Int = None,
        ...
        ended = await self._fetch_closed_order_rows(market, since, None, {} if params is None else params, filled)
        open_request: Dict[str, Any] = {'status': 'OPEN'}
        if market is not None:
            open_request['symbol'] = market['id']
        open_rows = [row for row in (self.safe_list(self.unwrap(await self.private_account_get_orders(open_request)), 'orders', []) or [])
                     if filled(row)]
        until = self.safe_integer(params, 'until')
        trades = self.parse_trades(ended + open_rows, market, since)
        if until is not None:
            trades = [trade for trade in trades if cast(int, trade['timestamp']) <= until]
    ...
    async def fetch_trades(self, symbol: str, since: Int = None, limit: Int = None,
        ...
        response = self.unwrap(await self.private_market_get_trades(self.extend(request, params)))
    ...
    def _parse_public_trade(self, trade: Dict[str, Any], market: MarketInterface) -> Trade:
    ...
    async def cancel_order(self, id: str, symbol: Str = None, params: Optional[Dict[str, Any]] = None) -> Order:
        ...
        response = self.unwrap(await self.private_account_post_orders_orderid_cancel({'orderId': id}))
        ...
        order = await self._confirm_cancel(id) if confirm else None
    ...
    async def _confirm_cancel(self, order_id: str) -> Any:
        ...
        for attempt in range(1, attempts + 1):
            if attempt > 1 and interval_ms > 0:
                await self.sleep(interval_ms)
            try:
                last = self.unwrap(await self.private_account_get_orders_orderid({'orderId': order_id}))
            except Exception:
    ...
    async def cancel_all_orders(self, symbol: Str = None, params: Optional[Dict[str, Any]] = None) -> List[Order]:
        ...
        for order in orders:
            ...
            try:
                canceled = await self.cancel_order(cast(str, order['id']), order.get('symbol'), {'trigger': trigger})
