...
class WatchHub:
    ...
    def next(self, hash: str) -> 'asyncio.Future[Any]':
        """`hash` 의 다음 값을 기다리는 Future. 실행 중인 이벤트 루프 안에서 부른다."""
    ...
    def resolve(self, hash: str, value: Any) -> None:
        ...
        for future in self._waiters.pop(hash, []):
            if not future.done():
    ...
    def push(self, hash: str, item: Any) -> None:
        ...
        items = self._buffers.get(hash, [])
        items.append(item)
        if len(items) > BUFFER_LIMIT:
            del items[:len(items) - BUFFER_LIMIT]
        if hash in self._waiters:
            self._buffers.pop(hash, None)
            self.resolve(hash, items)
        else:
