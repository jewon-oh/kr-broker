...
class KISCandleService:
    ...
    async def fetch_daily_ohlcv(self, stock_code: str, period_code: str, limit: int) -> List[List[float]]:
        ...
        rows = await self.fetch_daily_ohlcv_range(stock_code, period_code, start_date, to_kis_date(now))
        return rows[-limit:]
    ...
    async def fetch_overseas_daily_ohlcv(self, ticker: str, market: str, timeframe: str, limit: int, since: Optional[int] = None,
