...
def _format_local_date(ms: float) -> str:
    """UTC 밀리초의 지역 시간대 날짜 `YYYYMMDD`. 시각이 아니면 JavaScript 처럼 `NaNNaNNaN` 이다."""
        ...
        try:
            ...
            while len(all_candles) < limit:
                ...
                for item in data:
                    ...
                    all_candles.append([timestamp, _number(item, 'stck_oprc'), _number(item, 'stck_hgpr'), _number(item, 'stck_lwpr'),
                                        _number(item, 'stck_prpr'), _number(item, 'cntg_vol')])
            ...
            logger.info('[KISCandleService] fetchMinuteOHLCV 완료 (stockCode=%s, minuteInterval=%s, count=%d)', stock_code, minute_interval,
                        len(all_candles))
            ordered = sorted(all_candles, key=lambda c: c[0])
