...
async def build_extended_session_limit(source: Any, order: Dict[str, Any], label: str,
    ...
    """
    try:
        open_orders = await source.fetch_open_orders(order['symbol'])
    ...
    try:
        ticker = await source.fetch_ticker(order['symbol'])
