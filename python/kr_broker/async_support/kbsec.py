...
class KbsecAuth:
    ...
    def __init__(self, app_key: str, app_secret: str, base_url: str, post_json: Callable[[str, Any], Awaitable[Tuple[int, str]]],
    ...
    async def get_access_token(self) -> str:
        ...
        async with self._lock:
            ...
            if stored is not None:
                return stored
            return await refresh_token_with_lock('[KBSecAuth]', self.store_of(), self.store_key, TOKEN_FETCH_LOCK_TTL_MS,
    ...
    async def invalidate(self, failed_token: Str = None) -> None:
        ...
        if store is not None:
            try:
                if failed_token:
                    if not await maybe_await(store.delete_if_access_token_equals(self.store_key, failed_token)):
    ...
    async def rotate_after_token_failure(self, failed_token: str) -> None:
        ...
        """
        await self.invalidate(failed_token)
        reissued = await self.get_access_token()
        ...
        if not await self._revoke_token(failed_token):
            return
        # 폐기된 토큰은 누가 갖고 있든 쓸 수 없으므로 무조건 지운다.
        await self.invalidate()
        fresh = await self.get_access_token()
    ...
    async def _revoke_token(self, token: str) -> bool:
        ...
        for label, data_body in candidates:
            try:
                status, text = await self.post_json(f'{self.base_url}{KBSEC_REVOKE_PATH}', {'dataHeader': _EMPTY_HOST, 'dataBody': data_body})
    ...
    async def _issue_token(self) -> str:
        ...
        for shape in shapes:
            try:
                token = await self._request_token(shape)
    ...
    async def _request_token(self, shape: str) -> str:
        if shape == 'envelope':
        ...
        status, text = await self.post_json(f'{self.base_url}{KBSEC_TOKEN_PATH}', body)
        ...
        if store is not None and ttl_sec > 0 and not short_lived:
            try:
                await maybe_await(store.set(self.store_key, fn.json_stringify(self.cached_token), ttl_sec * 1000))
            except Exception:
...
class kbsec(Exchange, ImplicitAPI):
    ...
    async def _post_json(self, url: str, body: Any) -> Tuple[int, str]:
        ...
        response = await self.http_request('POST', url, {'Content-Type': 'application/json'}, self.json(body), AUTH_TIMEOUT_MS)
    ...
    async def authenticate(self, path: str, api: ApiName, method: str, params: Dict[str, Any], headers: Optional[Dict[str, str]],
        ...
        try:
            token = await self._get_auth().get_access_token()
        except BaseError:
            raise
    ...
    async def fetch2(self, path: str, api: ApiName = 'private', method: str = 'POST', params: Optional[Dict[str, Any]] = None,
        ...
        return await self._fetch_with_token_recovery(path, api, method, params, headers, body, config, True)

    async def _fetch_with_token_recovery(self, path: str, api: ApiName, method: str, params: Optional[Dict[str, Any]],
        ...
        try:
            response = await super().fetch2(path, api, method, params, request_headers, body, config)
        ...
        await self._get_auth().rotate_after_token_failure(failed_token)
        return await self._fetch_with_token_recovery(path, api, method, params, headers, body, config, False)
    ...
    async def _call_tr(self, tr_code: str, data_body: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        response = await method({} if data_body is None else data_body)
    ...
    async def fetch_investor_trading(self, symbol: str, since: Int = None, limit: Int = None,
        ...
        body = await self._call_tr(KBSEC_TR['INVESTOR_TRADING'], self.extend({
            'excg_clsf': '1',
            'is_cd': market['id'],
    ...
    async def _call_us_quote(self, base: str, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for krx_code in candidates:
            row = await self._call_tr(KBSEC_TR['QUOTE_US'], self.extend({'krx_cd': krx_code, 'is_cd': base}, params))
    ...
    async def fetch_ticker(self, symbol: str, params: Optional[Dict[str, Any]] = None) -> Ticker:
        ...
        if self._is_us(market):
            row = await self._call_us_quote(market_id, params)
            if row is None:
            ...
            row = await self._call_tr(KBSEC_TR['QUOTE_KR'], self.extend({'excg_clsf': '1', 'shrt_cd': market_id}, params))
    ...
    async def fetch_order_book(self, symbol: str, limit: Int = None, params: Optional[Dict[str, Any]] = None) -> OrderBook:
        ...
        if is_us:
            ...
            row = await self._call_tr(KBSEC_TR['ORDERBOOK_US'], self.extend({'krx_cd': krx_code, 'is_cd': market_id}, params))
        else:
            row = await self._call_tr(KBSEC_TR['ORDERBOOK_KR'], self.extend({'is_cd': market_id, 'ovtm_mkt_clsf': '1'}, params))
    ...
    async def fetch_ohlcv(self, symbol: str, timeframe: str = '1m', since: Int = None, limit: Int = None,
        ...
        body = await self._call_tr(KBSEC_TR['CHART_KR'], self.extend({
            'info_ccd': '1',  # 원주가
    ...
    async def fetch_trades(self, symbol: str, since: Int = None, limit: Int = None,
        ...
        market = self.market(symbol)
        if self._is_us(market):
            return await self._fetch_overseas_trades_timeline(market, since, limit, params)
        body = await self._call_tr(KBSEC_TR['TRADES_TIMELINE_KR'], self.extend({
            'excg_clsf': '1', 'is_cd': market['id'], 'ovtm_mkt_clsf': '0', 'inq_cnt': kbsec_num(30 if limit is None else limit),
        }, params))
        rows = pick_array(body)
        if len(rows) == 0:
            return []
        stamps = kst_trade_timestamps([pick_str(row, 'ccls_tm') for row in rows], await self._last_traded_date(market), self.milliseconds())
    ...
    async def _last_traded_date(self, market: MarketInterface) -> Str:
        ...
        symbol = market['symbol']
        try:
            candles = await self.fetch_ohlcv(symbol, '1d', None, 30, {'mkt_clsf': '1'} if self._is_kosdaq(market) else {})
        except Exception as err:
    ...
    async def _fetch_overseas_trades_timeline(self, market: MarketInterface, since: Int, limit: Int, params: Dict[str, Any]) -> List[Trade]:
        base = market['id'] or ''
        if self._us_exchange_cache.get(base) is None:
            await self._call_us_quote(base, {})
        krx_code = self._us_exchange_cache.get(base, KBSEC_US_EXCHANGES[0])
        body = await self._call_tr(KBSEC_TR['TRADES_TIMELINE_US'], self.extend({
    ...
    async def fetch_market_calendar(self, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        body = await self._call_tr(KBSEC_TR['MARKET_STATUS'], self.omit(params or {}, 'market'))
    ...
    async def fetch_order(self, id: str, symbol: Str = None, params: Optional[Dict[str, Any]] = None) -> Order:
        ...
        pending = await self._fetch_domestic_order_rows(KBSEC_CCLS_PENDING, market, self.safe_string(params, 'date'))
    ...
    async def fetch_my_trades(self, symbol: Str = None, since: Int = None, limit: Int = None,
        ...
        단가가 0 인 체결은 체결가를 모르므로 버린다. 체결 id 는 `주문번호#조회일자#순번` 이다(`_fill_rows_to_trades`).
        """
        params = {} if params is None else params
        if symbol is None:
        ...
        async def dated(ordr_dt: str) -> Dict[str, Any]:
            return {'date': ordr_dt, 'rows': await request(ordr_dt)}

        # 날짜를 주지 않으면 되감아 고른 영업일을 체결 id 에 넣어야 하므로, 조회한 날짜를 행과 함께 돌려받는다.
        if dates is None:
            return self._fill_rows_to_trades([await self._call_on_business_date(dated)], market, limit)
        days: List[Dict[str, Any]] = []
        for date in dates:
            days.append({'date': date, 'rows': await self._on_date_skipping_holiday(date, request, len(dates) > 1)})
    ...
    async def _fetch_overseas_trades(self, market: MarketInterface, dates: Optional[List[str]], limit: Int) -> List[Trade]:
        ...
        try:
            ...
            days: List[Dict[str, Any]] = []
            if dates is None:
                days.append(await self._call_on_business_date(dated, 'US'))
            else:
                for date in dates:
                    days.append({'date': date, 'rows': await self._on_date_skipping_holiday(date, request, len(dates) > 1)})
    ...
    def _fill_rows_to_trades(self, days: List[Dict[str, Any]], market: MarketInterface, limit: Int) -> List[Trade]:
        """체결 행을 체결 건별 `Trade` 로 옮긴다. `days` 는 조회일자(`date`, `YYYYMMDD`, 국내는 한국 날짜, 미국은 미국 현지 날짜)와 그날 받은 행(`rows`)이다.
        필드 이름은 `kbsec_fill_row` 가 정본이다.
        ...
        - 체결 id 는 `주문번호#조회일자#순번` 이다. 순번은 그날 그 주문의 체결을 행 순서대로 센 번호라서 조회 범위(`since`, `params['date']`)가 달라도 같다.
          조회일자를 넣었으므로 KB 가 주문번호를 날마다 새로 매겨도 다른 날의 체결과 겹치지 않는다. `trade['order']` 는 주문번호 그대로다(`create_order`, `fetch_order` 의 id).
        ...
        rows = [row for day in days for row in day['rows']]
        parsed_days = [(day['date'], [parse(row) for row in day['rows']]) for day in days]
        warn_if_fill_totals_inconsistent([parsed for _, day in parsed_days for parsed in day], rows, f"체결내역 {market['symbol']}")
        fill_days = [(date, [fill for fill in kbsec_resolve_fills(parsed) if base == '' or fill['symbol'] == '' or fill['symbol'] == base])
                     for date, parsed in parsed_days]
        trades: List[Trade] = []
        for date, fills in fill_days:
            counts: Dict[str, int] = {}
            for fill in fills:
                ...
                if not fill['price'] > 0:
                    ...
                    continue
                trades.append(self.parse_trade(self.extend(fill, {'date': date, 'index': index}), market))
        side_known = len([fill for _, fills in fill_days for fill in fills if fill['side'] is not None])
    ...
    def parse_trade(self, trade: Dict[str, Any], market: Market = None) -> Trade:
        """`kbsec_resolve_fills` 의 체결 건 하나(조회일자 `date` 와 순번 `index` 를 더한 것)를 체결로 옮긴다. id 는 `주문번호#조회일자#순번` 이고,
        순번은 그날 그 주문 안에서 센다."""
        return self.safe_trade({
            'id': f"{trade['orderId']}#{trade['date']}#{trade['index']}",
            'info': trade,
