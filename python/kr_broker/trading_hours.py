...
from kr_broker.us_market_hours import format_et_wall_clock, get_us_market_phase
...
def market_session_block_reason(exchange_id: str, symbol: str, now_ms: Optional[int] = None,
                                master_data: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    if is_krx_domestic_code(base):
        return trading_hours_block_reason(exchange_id, now)
    ...
    if group == 'US':
        ...
        phase = get_us_market_phase(now)
        if phase in ('open', 'closing-auction'):
            return None
        return f'미국장 정규장 외 ({format_et_wall_clock(now)}, phase={phase})'
