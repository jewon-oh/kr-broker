...
class kis(Exchange, ImplicitAPI):
    ...
    def _quote_market_division(self) -> str:
        ...
        return 'UN' if is_nxt_extended_tradable(self.milliseconds()) and self.is_option_enabled('nxtRouting') else 'J'
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = '1m', since: Int = None, limit: Int = None,
        ...
        try:
            ...
            yahoo = fetch_yahoo_candles(instrument.code, timeframe, limit, since, until, kr_market, exchange=self)
    ...
    def fetch_volatility_interruptions(self, symbol: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    def fetch_stock_warnings(self, symbol: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        return self.fetch_volatility_interruptions(symbol, params)

    def fetch_investor_trading(self, symbol: str, since: Any = None, limit: Int = None,
    ...
    def fetch_balance(self, params: Optional[Dict[str, Any]] = None) -> Balances:
        ...
        `'usd'` 는 달러 예수금과 평가금액(`USD`)이다. 응답에 달러 행이 없으면 `USD` 항목이 없다(0 이 아니라 모른다는 뜻이다).
        `params['orderable']` 이 `False` 가 아니면 국내 주문가능현금을 한 번 더 조회해 `KRW` 의 `free` 로 쓴다. 원본 응답은 `info` 에
        `{'domestic', 'overseas', 'usd'}` 로 담는다.
    ...
    def parse_balance(self, response: Any) -> Balances:
        ...
        if domestic is not None:
            ...
            deposit = self.safe_string(summary, 'dnca_tot_amt')
            free = None if orderable is None else self.safe_string(orderable, 'ord_psbl_cash')
            # `used` 는 `safe_balance` 가 `total − free` 로 채운다.
            total = None if Precise.string_gt(free, deposit) else deposit
            result['KRW'] = {'free': free, 'used': None, 'total': total, 'info': {'summary': summary, 'orderable': orderable}}
            for item in rows_of(domestic.get('holdings')):
                self._add_holding(holdings, item, 'pdno', 'hldg_qty')
        overseas = self.safe_dict(response, 'overseas')
        if overseas is not None:
            for item in rows_of(overseas.get('holdings')):
                self._add_holding(holdings, item, 'ovrs_pdno', 'ovrs_cblc_qty')
    ...
    def _create_domestic_order(self, instrument: KisInstrument, type: str, side: str, quantity: int, price: Num,
        ...
        extended = session == 'nxt' or (session is None and self.is_option_enabled('nxtRouting') and is_nxt_extended_tradable(self.milliseconds()))
        if extended:
    ...
    def _assert_domestic_session_open(self, side: str) -> None:
        ...
        self.refresh_market_calendar()
        reason = krx_order_block_reason(self.milliseconds(), side, self.is_option_enabled('blockAuctionBuys'))
        if reason is not None:
            raise MarketClosed(reason)
    ...
    def _create_overseas_order(self, instrument: KisInstrument, type: str, side: str, quantity: int, price: float,
        ...
        if exchange in US_ORDER_EXCHANGES:
            reason = us_order_block_reason(self.milliseconds(), side, self.is_option_enabled('blockAuctionBuys'))
            if reason is not None:
                raise MarketClosed(reason)
    ...
    def _cancel_overseas_order(self, id: str, instrument: KisInstrument, params: Optional[Dict[str, Any]]) -> Order:
        exchange = instrument.order_exchange
        if exchange is None:
            raise BadSymbol(f'해외 마스터에 없는 ticker: {instrument.symbol}')
    ...
    def edit_order(self, id: str, symbol: str, type: str, side: str, amount: Num = None, price: Num = None,
        ...
        instrument = self._instrument_of(symbol)
        if instrument.overseas:
            return self._edit_overseas_order(id, instrument, price, amount, partial, params)
        ...
        if not partial and amount is not None:
            assert_whole_remaining_edit(self.id, id, amount, self._find_open_order(instrument, id))
        response = self.private_post_uapi_domestic_stock_v1_trading_order_rvsecncl(self.extend(self.extend(self._account_params(), {
            'KRX_FWDG_ORD_ORGNO': self.safe_string(params, 'orderOrgNo', ''),
            'ORGN_ODNO': id,
            'ORD_DVSN': KIS_ORDER_TYPE['LIMIT'],
    ...
    def _find_open_order(self, instrument: KisInstrument, id: str) -> Order:
    ...
    def _edit_overseas_order(self, id: str, instrument: KisInstrument, price: float, amount: Num, partial: bool,
                             params: Optional[Dict[str, Any]]) -> Order:
        ...
        total = None
        if quantity is None:
            open_order = self._find_open_order(instrument, id)
            if amount is not None:
    ...
    def cancel_all_orders(self, symbol: Str = None, params: Optional[Dict[str, Any]] = None) -> List[Order]:
        ...
        for order in open_orders:
            try:
                canceled = self.cancel_order(cast(str, order['id']), order.get('symbol'), self._cancel_params_of(order))
