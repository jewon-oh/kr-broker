"""KB증권 `processCode` → 오류 클래스 표. TypeScript 판 `ts/src/kbsec/kbsec-error-codes.ts` 와 같다(두 판의 표는 `test_ts_table_parity.py` 가 대조한다).

KB 는 코드 체계를 공개하지 않는다. 그래서 모든 항목은 실측한 것이고, 표에 없는 코드는 `ExchangeError` 로 남는다.
`1861`(조회할 자료가 없습니다)과 `2149`(해당자료가 없습니다)는 `kbsec_envelope` 가 정상적인 빈 결과로 보므로 이 표에 없다.
클래스만으로 원인을 다 말하지 못하는 항목은 `detail` 을 함께 두고, 던진 오류의 `detail` 에 싣는다.
"""
...
KBSEC_ERROR_DETAIL = {
    ...
    # 매도할 보유 수량이 모자란다(`InsufficientFunds` 로 던진다).
...
# 실측한 processCode 의 매핑. 값은 `{'error': 오류 클래스, 'detail': detail}` 이고 `detail` 은 없을 수 있다.
KBSEC_PROCESS_CODES: Dict[str, Dict[str, Any]] = {
    # 토큰 검증 실패(SSQM0004 실측). 재발급과 1회 재시도로 복구되고, 재시도 뒤에도 남으면 인증 실패다.
    ...
    # `앱키로 앱정보 추출 중 오류...` — 앱키가 틀렸을 때 실측.
    'E021': {'error': AuthenticationError},
    # `증거수량이 부족하여 주문불가합니다` — SSAM1801 매도 재시도 실측.
    ...
    # `시장 구분값을 확인하세요` — `mkt_tm_ccd` 를 빠뜨려 거부된 사례(SSQM1801).
    '3576': {'error': BadRequest},
    # `체결구분을 확인하십시오` — 구분 필드를 비웠을 때 실측.
    '8654': {'error': BadRequest},
    # `글로벌원마켓 미신청계좌입니다` — SPQM3390 실측. 계좌 가입이 필요하다.
    'H049': {'error': AccountNotEnabled},
    # `NXT에서 거래할 수 없는 종목입니다. KRX로 주문해주세요.` — SOR 로 보낸 우선주 등에서 실측. 접수되지 않은 거부다.
...
def kbsec_exact_exceptions() -> Dict[str, Any]:
    """`exceptions['exact']` 에 넣는 표."""
    return {code: mapping['error'] for code, mapping in KBSEC_PROCESS_CODES.items()}


def kbsec_error_detail(process_code: str) -> Optional[str]:
    """이 processCode 의 `detail`. 없으면 `None`."""
    mapping = KBSEC_PROCESS_CODES.get(process_code.strip())
    return None if mapping is None else mapping.get('detail')
