...
- KB증권 `fetchMyTrades`의 체결 id를 `주문번호#순번`에서 `주문번호#조회일자#순번`으로 바꿉니다(예: `0000000025#20260922#0`). 조회일자는 국내가 한국 날짜, 미국이 미국 현지 날짜입니다. 날짜를 주지 않고 부르면 휴장일을 되감아 실제로 조회한 영업일이 들어갑니다. 순번은 그날 그 주문의 체결 가운데 몇 번째인지로 셉니다. 예전에는 조회한 기간 전체에서 센 위치라서 같은 체결의 id가 조회 범위에 따라 바뀌었습니다. `since`로 여러 날을 조회하면 `#1`이던 체결이 그날만 조회하면 `#0`이었고, 같은 날 앞 주문의 체결 수만큼 뒤 주문의 순번이 밀렸습니다. 이제 id에 조회일자가 들어가므로 KB가 주문번호를 날마다 새로 매겨도 다른 날의 체결과 겹치지 않습니다. `trade.order`는 예전처럼 주문번호이고, `info`에는 조회일자(`date`)가 더해집니다. 분할체결 연속 행은 같은 조회일자의 앞 주문에만 귀속합니다. 같은 주문의 새 체결이 그 주문의 행 가운데 어디에 붙는지는 실계좌로 확인하지 못했습니다. 앞에 붙는다면 그날 체결이 더 생길 때 먼저 받은 체결의 순번이 밀립니다. 예전 id를 저장해 둔 코드는 새 id와 맞지 않으므로 다시 받습니다. Python 판도 같습니다.
...
- KB증권 국내 `fetchOrder`에 `params.date`를 주면 주문 목록도 그날로 조회합니다. 예전에는 체결내역만 그날을 조회하고 미체결 목록은 가장 최근 영업일을 조회했습니다. 그래서 두 조회가 서로 다른 날을 보았고, 두 날에 같은 주문번호가 있으면 서로 다른 주문을 하나로 합쳤습니다. 지난 날짜의 주문 목록에 그날 끝까지 체결되지 않은 미체결수량이 남아 오는지는 실계좌로 확인하지 못했습니다. 남아 온다면 그 주문은 `fetchOrders`에 같은 날짜를 줄 때처럼 `open`으로 나옵니다. `params.date`를 주지 않으면 예전과 같습니다. Python 판도 같습니다.
