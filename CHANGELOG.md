...
- KB증권 국내 `fetchOrder`에 `params.date`를 주면 미체결 목록도 그날로 조회합니다. 예전에는 체결내역만 그날을 조회하고 미체결 목록은 가장 최근 영업일을 조회했습니다. 그래서 두 조회가 서로 다른 날을 보았고, 두 날에 같은 주문번호가 있으면 서로 다른 주문을 하나로 합쳤습니다. 지난 날짜의 미체결 목록에 그날 끝까지 체결되지 않은 주문이 남아 오는지는 실계좌로 확인하지 못했습니다. 남아 온다면 그 주문은 `fetchOrders`에 같은 날짜를 줄 때처럼 `open`으로 나옵니다. `params.date`를 주지 않으면 예전과 같습니다. Python 판도 같습니다.
