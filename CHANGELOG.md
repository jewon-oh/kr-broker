...
- KB증권 `fetchMyTrades`의 체결 id(`주문번호#순번`)에서 순번을 그날 그 주문의 체결 가운데 몇 번째인지로 셉니다. 예전에는 조회한 기간 전체에서 센 위치라서 같은 체결의 id가 조회 범위에 따라 바뀌었습니다. 예를 들어 `since`로 여러 날을 조회하면 `#1`이던 체결이 그날만 조회하면 `#0`이었고, 같은 날 앞 주문의 체결 수만큼 뒤 주문의 순번이 밀렸습니다. 분할체결 연속 행은 같은 조회일자의 앞 주문에만 귀속합니다. 같은 주문의 새 체결이 그 주문의 행 가운데 어디에 붙는지는 실계좌로 확인하지 못했습니다. 앞에 붙는다면 그날 체결이 더 생길 때 먼저 받은 체결의 순번이 밀립니다. 주문번호를 날마다 새로 매기는지도 확인하지 못했습니다. 새로 매긴다면 여러 날을 조회한 결과에서 다른 날의 체결이 같은 id를 가질 수 있습니다. 예전 id를 저장해 둔 코드는 새 id와 맞지 않으므로 다시 받습니다. Python 판도 같습니다.
