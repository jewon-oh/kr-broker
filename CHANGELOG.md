...
- 범위를 넘는 시각(`240000`)을 그날 0시로 읽던 봉 경로를 더 고쳤습니다. KB증권 국내 `fetchOHLCV`는 이런 행을 건너뜁니다. 한국투자증권 `fetchIndexOHLCV`의 분봉과 `fetchMinuteOHLCVAt`, `fetchOverseasMinuteOHLCV`도 건너뜁니다. 한국투자증권 `fetchExpectedPriceTrend`의 추이와 KB증권 `fetchTrades`의 체결은 행을 남기고 `timestamp`를 비웁니다.
