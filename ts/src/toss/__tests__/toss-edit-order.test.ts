 ...
 * 장 시간 게이트의 경계 시각은 두 판이 함께 도는 요청 픽스처(`test/static/request/toss.json`)가 고정한다.
 */
...
/** 지금이 정규장 안에 드는 세션 시간. */
const regularNow = () => ({ startTime: new Date(Date.now() - HOUR).toISOString(), endTime: new Date(Date.now() + 3 * HOUR).toISOString() });
/** 정정도 주문처럼 원주문 조회 전에 장 운영 캘린더로 장 시간을 판정한다. 테스트를 언제 돌려도 정규장이도록 지금을 감싸는 캘린더를 준다. */
const OPEN_CALENDARS: Record<string, Route> = {
    'GET /api/v1/market-calendar/KR': () => jsonOk({ today: { date: '2026-08-03', integrated: { regularMarket: regularNow() } } }),
    'GET /api/v1/market-calendar/US': () => jsonOk({ today: { date: '2026-08-03', regularMarket: regularNow() } }),
};
...
describe('editOrder — 국내', () => {
    it('지정가 — 주문 상세의 잔량을 수량으로 싣고 가격을 정정해 새 orderId 를 돌려준다. amount 는 정정 뒤 총수량이다', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': krOrder('3', '0'), 'POST /api/v1/orders/O1/modify': jsonOk({ orderId: 'O2' }) });

        const order = await makeToss().editOrder('O1', '005930/KRW', 'limit', 'buy', 3, 71000);

        expect(fake.requests().map((r) => `${r.method} ${r.path}`)).toEqual([
            'GET /api/v1/market-calendar/KR', 'GET /api/v1/orders/O1', 'POST /api/v1/orders/O1/modify',
        ]);
    ...
    it('★일부 체결된 주문은 quantity 의 뜻을 확인하기 전까지 정정 요청 없이 NotSupported 다', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': krOrder('3', '1'), 'POST /api/v1/orders/O1/modify': jsonOk({ orderId: 'O2' }) });
    ...
    it('주문 상세 조회가 실패하면 정정 요청 없이 던진다. 추정한 잔량으로 정정하지 않는다', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': errorReply(500, 'internal-error'), 'POST /api/v1/orders/O1/modify': jsonOk({ orderId: 'O2' }) });
    ...
    it('1억원 이상은 confirmHighValueOrder 를 켠다. 명목가는 잔량 × price 다', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': krOrder('2000', '0'), 'POST /api/v1/orders/O1/modify': jsonOk({ orderId: 'O2' }) });
...
describe('editOrder — 해외', () => {
    it('지정가 — 가격만 정정한다(수량 필드 없음)', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': usOrder('3', '1'), 'POST /api/v1/orders/O1/modify': jsonOk({ orderId: 'O2' }) });
    ...
    it('정정 전에 주문 상세를 읽어 남은 수량 × 새 가격이 고액이면 confirmHighValueOrder 를 켠다', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': usOrder('400', '50'), 'POST /api/v1/orders/O1/modify': jsonOk({ orderId: 'O2' }) });

        await makeToss().editOrder('O1', 'AAPL/USD', 'limit', 'buy', undefined, 229.5);

        expect(fake.requests().map((r) => `${r.method} ${r.path}`)).toEqual([
            'GET /api/v1/market-calendar/US', 'GET /api/v1/orders/O1', 'POST /api/v1/orders/O1/modify',
        ]);
    ...
    it('체결된 수량은 빼고 본다: 남은 수량의 명목가가 기준 아래면 켜지 않는다', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': usOrder('400', '390'), 'POST /api/v1/orders/O1/modify': jsonOk({ orderId: 'O2' }) });
    ...
    it('정정 요청이 연결 오류로 끝나면 OrderOutcomeUnknown 이고 정정 요청을 다시 보내지 않는다', async () => {
        const fake = installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': usOrder('400', '50'), 'POST /api/v1/orders/O1/modify': () => networkFailure('ECONNRESET') });
...
describe('editOrder — 공통', () => {
    ...
    it('응답에 orderId 가 없으면 OrderOutcomeUnknown', async () => {
        installFakeToss({ ...OPEN_CALENDARS, 'GET /api/v1/orders/O1': krOrder('3', '0'), 'POST /api/v1/orders/O1/modify': jsonOk({}) });
