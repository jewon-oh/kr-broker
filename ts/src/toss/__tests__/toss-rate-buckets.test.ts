...
describe('그룹 버킷', () => {
    ...
    it('주문 요청만 order 로 표시되고, 주문 요청은 재시도·시간 상한에서 조회와 다르게 다뤄진다', () => {
        ...
        expect(orders).toEqual([
            ...
            'POST conditional-orders/{conditionalOrderId}/modify',
            'POST orders',
    ...
    it('개장 직후 한도가 줄어드는 그룹(매수여력·수수료·매도가능수량)만 peak 로 표시된다. 주문 그룹은 줄지 않는다', () => {
        const peak = endpoints().filter((e) => e.config.peak === true).map((e) => e.path).sort();
        expect(peak).toEqual(['buying-power', 'commissions', 'sellable-quantity']);
    });
