 ...
 * (`kbsec-fill-row.ts` 헤더의 정본 표). 그래서 체결 TR 로 만든 `Trade.fee` 는 공시 요율에서 뽑은
 * **추정치**다(`kbsec-fee.ts`). KB 가 실제로 청구한 금액을 주는 곳은 정산 TR 뿐이다.
 *
 ...
 *
 * `trd_clsf` 를 `1` 로 보내면 **매도만**, `2` 로 보내면 매수만 온다. `9`(전체)는 실응답이 **잘린 부분집합**이라
 * 쓰지 않는다. 양쪽 비용을 다 받으려면 `1` 과 `2` 를 따로 부른다(`kbsec.fetchDomesticSettlements`).
 * 값은 {@link KBSEC_SETTLE_TRD_CLSF} 에 둔다.
 *
 ...
 * 페이지 처리는 어댑터(`kbsec.fetchDomesticSettlements`)가 한다.
 *
...
/** `trd_clsf`(매매구분) 입력 코드 — `9` 전체 · `1` 매도 · `2` 매수. `ALL`(`9`)은 실응답이 잘려 비용 행 일부가 빠지므로 `SELL`·`BUY` 를 따로 부른다. */
