...
- `pnpm typecheck`는 `examples/`도 검사합니다. README의 예제 코드가 실제 API와 어긋나면 `pnpm typecheck`가 실패합니다.
- 테스트 하나만 실행하려면 `pnpm exec vitest run src/kis/__tests__/kis-order.test.ts`처럼 경로를 지정합니다.
- 테스트는 `src/**/__tests__/` 아래에 둡니다.
...
세 증권사는 같은 상황에서 같은 계약을 지켜야 합니다. 계약은 공통 스위트가 코드로 정의합니다. TypeScript 구현의 위치는 `src/__tests__/support/broker-contract-suite.ts`입니다.
...
3. 증권사 하네스(`BrokerContractHarness`)를 만들어 `defineBrokerContractSuite`를 호출합니다. `src/kis/__tests__/kis-contract.test.ts`가 예입니다.
...
1. `has`에 기능 이름을 적고 메서드를 구현합니다. `has`와 구현이 어긋나면 `src/__tests__/broker-capabilities.test.ts`가 실패합니다.
